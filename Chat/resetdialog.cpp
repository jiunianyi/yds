...
void ResetDialog::on_varify_btn_clicked()
{
    ...
    HttpMgr::getinstance()->posthttpreq(QUrl(gate_url_prefix+"/get_varifycode"),
                                        json_obj, RedId::ID_GET_VARIFY_CODE,Modules::RESETMOD);
}

void ResetDialog::slot_reset_mod_finish(RedId id, QString res, ErrorCodes err)
{
...
{
    //注册获取验证码回包逻辑
    _handlers.insert(RedId::ID_GET_VARIFY_CODE, [this](QJsonObject jsonObj){
    ...
    //注册注册用户回包逻辑
    _handlers.insert(RedId::ID_RESET_PWD, [this](QJsonObject jsonObj){
        int error = jsonObj["error"].toInt();
        if(error != ErrorCodes::SUCCESS){
            showTip(tr("参数错误"),false);
            return;
...
void ResetDialog::on_sure_btn_clicked()
{
    ...
    HttpMgr::getinstance()->posthttpreq(QUrl(gate_url_prefix+"/reset_pwd"),
                 json_obj, RedId::ID_RESET_PWD,Modules::RESETMOD);
