    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    //显示更多标签
    connect(ui->more_lb, &Clickedable::clicked, this, &ApplyFriend::ShowMoreLabel);
    InitTipLbs();
    //链接输入标签回车事件
    ...
    connect(ui->tip_lb, &Clickedable::clicked, this, &ApplyFriend::SlotAddFirendLabelByClickTip);
...
void ApplyFriend::addLabel(QString name)
{
    ...
    connect(tmplabel, &Friendlable::sig_close, this, &ApplyFriend::SlotRemoveFriendLabel);
...
void ApplyFriend::SlotApplySure()
{
    ...
    if(backname.isEmpty())
    {
        backname = ui->name_ed->placeholderText();
    }
    jsonObj["backname"] = backname;
    jsonObj["touid"] = _si->_uid;//被搜索人的uid
    QJsonDocument doc(jsonObj);
    QByteArray jsondata = doc.toJson(QJsonDocument::Compact);
    //发送tcp请求给chatserver
    emit TcpMgr::getinstance()->sig_send_data(RedId::ID_ADD_FRIEND_REQ,jsondata);
    this->hide();
    deleteLater();
}
